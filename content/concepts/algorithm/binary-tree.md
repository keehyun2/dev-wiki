...
4. 이진트리를 자연 매핑 하였을 때 노드 i 의 부모의 인덱스는 **(i - 1) / 2** 이고, 자식은 **2 * i + 1 과 2 * i + 2** 입니다. 
   ...
   2 * i + 2 // node i 's right child index 
   ```
...
이진 트리의 크기 n 이 주어졌을 때 **서로 다른 이진트리의 개수**를 구할 수가 있습니다. 크기 0 or 1일때 서로 다른 이진트리의 개수는 1로 정의하고, 나머지 크기에 따른 이진트리의 개수는 공식으로 구할 수 있습니다.  
